...
/*   Updated: 2026/03/31 18:57:48 by dkalgano         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool    mlx_scale_image_into(mlx_image_t *src, mlx_image_t *dst)
{
    ...
    if (!src || !dst || !src->pixels || !dst->pixels)
        return (false); // Or your mlx_error(MLX_INVIMG)
    if (!src->width || !src->height || !dst->width || !dst->height)
        return (false); // Or your mlx_error(MLX_INVDIM)
...
static void fill_pixels(mlx_image_t *src, mlx_image_t *dst)
{
    ...
    while (y < dst->height)
    {
        // Calculate the starting index of the source row once per Y-iteration
        s_off = ((y * src->height) / dst->height) * src->width;
        x = 0;
        while (x < dst->width)
        {
            // Calculate X offset with integer math and increment destination pointer
